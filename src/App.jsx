...
function App() {
    ...
    if (e.key === 'Enter') {
      ...
      if (standardLength === null) {
        ...
        if (serials.includes(value)) {
          ...
          handleError(`SAI ĐỊNH DẠNG: Yêu cầu ${standardLength} ký tự (Mã quét: ${value.length})`);
        } else {
          addSerial(value);
        }
      }
  ...
  const handleReset = () => {
    if (window.confirm('Bạn có chắc muốn xóa toàn bộ danh sách và làm lại từ đầu?')) {
      setSerials([]);
  ...
  const handleGenerate = () => {
    ...
    for (let i = 0; i < count; i++) {
      const currentNumStr = (startNum + i).toString().padStart(paddingLength, '0');
    ...
    if (standardLength === null) {
      ...
      const validSerials = newSerials.filter(s => s.length === standardLength);
      if (validSerials.length !== newSerials.length) {
        showToast(`Bỏ qua ${newSerials.length - validSerials.length} mã sai độ dài chuẩn (${standardLength})`, 'error');
      }
      newSerials.length = 0;
  ...
  return (
    <div className={`app-container ${flash ? `flash-${flash}` : ''}`}>
        ...
        <div className="generator-container">
          <div className="generator-inputs">
             <div className="input-group">
                ...
                <input type="text" value={genStart} onChange={e => setGenStart(e.target.value)} className="gen-input" placeholder="VD: SN-001" />
             </div>
             <div className="input-group">
                <label>Số lượng</label>
                <input type="number" min="1" max="5000" value={genCount} onChange={e => setGenCount(e.target.value)} className="gen-input" />
             </div>
          </div>
      ...
      <div className="actions">
        <button className="btn btn-primary" onClick={handleCopy} disabled={serials.length === 0}>
        ...
        <button className="btn btn-danger" onClick={handleReset} disabled={serials.length === 0 && standardLength === null}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="list-container">
        ...
        <ul className="serial-list">
          ...
          {serials.length === 0 && (
            <li className="serial-item" style={{ justifyContent: 'center', color: 'var(--text-secondary)' }}>
